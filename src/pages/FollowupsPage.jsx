...
export default function FollowupsPage({ user }) {
          ...
          {events.map(ev => {
            ...
            return (
              <button key={ev.id} onClick={() => openEvent(ev)}
                className="text-left rounded-3xl border border-[#E4EBF3] bg-white p-6 shadow-xs transition-all hover:border-[#003158] hover:shadow-md">
                <div className="flex items-center justify-between mb-3">
                  <span className="rounded-full bg-[#EAF0F7] px-3 py-1 text-[11px] font-bold text-[#1F3A5F]">{ev.type || 'Sabha'}</span>
                  <span className="text-xs font-semibold text-[#9BB5CB]">{ev.date}</span>
                </div>
                <h3 className="text-base font-bold text-[#003158] mb-1">{ev.title}</h3>
                ...
                <div className="pt-3 border-t border-[#F0F4F8] flex items-center justify-between">
                  <span className="text-xs text-slate-500"><span className="font-extrabold text-[#003158]">{done}</span> contacted</span>
  ...
  const chips = [
    { label: 'Audience', value: summary.total, cls: 'text-[#003158]' },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 space-y-5">
      <button onClick={() => setSelectedEvent(null)} className="flex items-center gap-1.5 text-xs font-bold text-[#003158] hover:text-[#FF862A]">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-[#003158]">{selectedEvent.title}</h1>
        </div>
        <p className="text-sm font-medium text-[#9BB5CB]">{selectedEvent.date} • {selectedEvent.time} • {selectedEvent.venue}</p>
      </div>

      {/* Summary chips */}
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
        {chips.map(c => (
          <div key={c.label} className="rounded-2xl border border-[#E4EBF3] bg-white px-3 py-2 text-center shadow-xs">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowTagPanel(v => !v)}
              className={`flex items-center gap-2 rounded-2xl border px-4 py-2.5 text-sm font-bold ${selectedTags.length ? 'border-[#003158] bg-[#EAF0F7] text-[#003158]' : 'border-[#E0EAF4] bg-white text-[#9BB5CB]'}`}>
              <Filter className="h-4 w-4" /> Tags{selectedTags.length ? ` (${selectedTags.length})` : ''}
            </button>
            <button onClick={() => setPendingOnly(v => !v)}
              className={`rounded-2xl border px-4 py-2.5 text-sm font-bold ${pendingOnly ? 'border-[#FF862A] bg-amber-50 text-[#C8642B]' : 'border-[#E0EAF4] bg-white text-[#9BB5CB]'}`}>
              Pending only
            </button>
          </div>
        </div>
                  ...
                  {tags.map(t => {
                    ...
                    return (
                      <button key={t.key} onClick={() => toggleTag(t.key)}
                        ...
                        className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${on ? '' : 'border border-[#E4EBF3] text-slate-500'}`}>
                        {t.label}
                      </button>
                    );
                  })}
        ...
        {audience.slice(0, AUDIENCE_CAP).map(d => {
          ...
          const Tick = ({ on, onClick, icon: Icon, label }) => (
            <button onClick={onClick} title={label}
              className={`flex h-8 w-8 items-center justify-center rounded-xl border transition-all ${on ? 'border-[#003158] bg-[#003158] text-white' : 'border-[#E4EBF3] bg-white text-slate-400 hover:border-[#003158]'}`}>
              <Icon className="h-4 w-4" />
            </button>
          );
          return (
            <div key={d.id} className="rounded-2xl border border-[#E4EBF3] bg-white p-3 shadow-xs">
              ...
              <input
                ...
                className="mt-2 w-full rounded-xl border border-[#EEF2F7] bg-[#F8FAFC] px-3 py-1.5 text-xs text-[#003158] outline-none focus:border-[#003158]" />
            </div>
          );
        })}

        {audience.length === 0 && (
          <p className="text-center text-xs text-[#9BB5CB] py-8">No devotees match this filter.</p>
        )}
        {audience.length > AUDIENCE_CAP && (
          <p className="text-center text-xs text-[#9BB5CB]">Showing first {AUDIENCE_CAP} of {audience.length}. Use search or tags to narrow the audience.</p>
        )}
      </div>
    </div>
  );
