  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-main">Sabhas & Attendance</h1>
        ...
        {(user?.role === 'Admin' || user?.role === 'Sevak') && (
          <button
            ...
            className="flex items-center gap-2 rounded-2xl bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-md hover:bg-[#00223f] transition-all"
          >
        ...
        {sabhas.map((sabha) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs font-semibold text-text-muted">{sabha.date}</span>
            </div>

            <h3 className="text-base font-bold text-text-main mb-1">{sabha.title}</h3>
            ...
            <div className="pt-3 border-t border-border-light flex items-center justify-between">
              <div>
                <span className="text-lg font-extrabold text-text-main">{sabha.presentCount}</span>
                <span className="text-xs text-slate-400"> / {sabha.totalCount} Devotees</span>
              </div>

              <button
                onClick={() => handleOpenAttendance(sabha)}
                className="flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-xs font-bold text-white hover:bg-[#00223f]"
              >
      ...
      {activeSabha && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-2xl rounded-3xl bg-surface p-6 shadow-2xl flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between border-b border-border-light pb-4 mb-4">
              <div>
                <h2 className="text-lg font-bold text-text-main">{activeSabha.title}</h2>
                <p className="text-xs text-slate-400">{activeSabha.date} • Check mark present devotees</p>
              </div>
              <button onClick={() => setActiveSabha(null)} className="text-text-muted hover:text-text-main">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-border-light bg-bg-base pl-10 pr-4 py-2.5 text-xs font-semibold text-text-main outline-none"
              />
            </div>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-2xl border cursor-pointer transition-all ${
                      isPresent ? 'border-emerald-500 bg-emerald-50/50' : 'border-border-light bg-surface'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="text-xs font-bold text-text-main">{dev.name}</p>
            ...
            <div className="border-t border-border-light pt-4 mt-4 flex items-center justify-between">
              <span className="text-xs font-bold text-text-main">
                {Object.values(attendanceState).filter(Boolean).length} / {devotees.length} Marked Present
              </span>
              <button
                onClick={handleSaveAttendance}
                className="rounded-2xl bg-primary px-6 py-2.5 text-xs font-bold text-white shadow-md hover:bg-[#00223f]"
              >
                Save Attendance
              </button>
            </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-3xl bg-surface p-6 shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-border-light pb-4 mb-4">
              <h2 className="text-lg font-bold text-text-main">Schedule Sabha</h2>
            ...
            <form onSubmit={handleCreateSabha} className="space-y-3">
              <div>
                <label className="block text-xs font-bold text-text-main mb-1">Sabha Title *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full rounded-2xl border border-border-light p-3 text-xs font-semibold outline-none focus:border-primary"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-text-main mb-1">Date</label>
                  <input
                    type="date"
                    value={newSabha.date}
                    onChange={(e) => setNewSabha({ ...newSabha, date: e.target.value })}
                    className="w-full rounded-2xl border border-border-light p-3 text-xs font-semibold outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-text-main mb-1">Time</label>
                  <input
                    type="text"
                    value={newSabha.time}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-text-main mb-1">Venue</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-text-main mb-1">Type</label>
                  <select
                    value={newSabha.type}
                    onChange={(e) => setNewSabha({ ...newSabha, type: e.target.value })}
                    className="w-full rounded-2xl border border-border-light p-3 text-xs font-semibold outline-none bg-surface"
                  >
              ...
              <button
                type="submit"
                className="w-full rounded-2xl bg-primary py-3 text-xs font-bold text-white shadow-md hover:bg-[#00223f] mt-2"
              >
                Create Sabha
              </button>
            </form>
